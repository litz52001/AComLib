package com.acommonlibrary.appupdate.service;
...
import com.acommonlibrary.appupdate.base.BaseHttpDownloadManager;
import com.acommonlibrary.appupdate.base.Constant;
import com.acommonlibrary.appupdate.config.UpdateConfiguration;
import com.acommonlibrary.appupdate.listener.OnDownloadListener;
import com.acommonlibrary.appupdate.manager.DownloadManager;
import com.acommonlibrary.appupdate.manager.HttpDownloadManager;
import com.acommonlibrary.appupdate.utils.NotificationUtil;
import com.acommonlibrary.util.AppUtils;
...
public final class DownloadService extends Service implements OnDownloadListener {
    ...
    private void init() {
        ...
        if (downloadManager == null) {
            MLog.d(TAG, "init DownloadManager.getInstance() = null ,请先调用 getInstance(Context context) !");
            return;
        }
        ...
        MLog.d(TAG, enable ? "应用的通知栏开关状态：已打开" : "应用的通知栏开关状态：已关闭");
        if (checkApkMD5()) {
            MLog.d(TAG, "文件已经存在直接进行安装");
            //直接调用完成监听即可
            done(FileUtils.createFile(downloadPath, apkName));
        } else {
            MLog.d(TAG, "文件不存在开始下载");
    ...
    private boolean checkApkMD5() {
        if (FileUtils.fileExists(downloadPath,apkName)) {
            String fileMD5 = FileUtils.getFileMD5ToString(downloadPath+File.separator+ apkName);
    ...
    private synchronized void download(UpdateConfiguration configuration) {
        if (downloadManager.isDownloading()) {
            MLog.e(TAG, "download: 当前正在下载，请务重复下载！");
            return;
        }
    ...
    public void downloading(int max, int progress) {
        MLog.i(TAG, "max: " + max + " --- progress: " + progress);
        if (showNotification) {
            //优化通知栏更新，减少通知栏更新次数
    ...
    public void done(File apk) {
        MLog.d(TAG, "done: 文件已下载至" + apk.toString());
        ...
        if (jumpInstallPage) {
            AppUtils.installApk(this, Constant.AUTHORITIES, apk);
        }
