package com.acommonlibrary.appupdate.manager;
...
import com.acommonlibrary.appupdate.dialog.UpdateDialog;
...
public class DownloadManager {
    ...
    public void download() {
        ...
        if (checkVersionCode()) {
            ...
            //对版本进行判断，是否显示升级对话框
            if (apkVersionCode > AppUtils.getAppVersionCode(context)) {
    ...
    private boolean checkParams() {
        if (TextUtils.isEmpty(apkUrl)) {
            MLog.e(TAG, "apkUrl can not be empty!");
            return false;
        }
        if (TextUtils.isEmpty(apkName)) {
            MLog.e(TAG, "apkName can not be empty!");
            return false;
        }
        if (!apkName.endsWith(Constant.APK_SUFFIX)) {
            MLog.e(TAG, "apkName must endsWith .apk!");
            return false;
        }
        downloadPath = context.getExternalCacheDir().getPath();
        if (smallIcon == -1) {
            MLog.e(TAG, "smallIcon can not be empty!");
            return false;
        }
        //加载用户设置的authorities
    ...
    private boolean checkVersionCode() {
        ...
        if (TextUtils.isEmpty(apkDescription)) {
            MLog.e(TAG, "apkDescription can not be empty!");
        }
        return false;
    }
